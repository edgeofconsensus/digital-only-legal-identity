...
## 21. Conformance analysis

If a concrete external implementation, legislative proposal, API, or public system design based on the DOLI concept emerges, compatibility SHOULD be evaluated against the protocol invariants defined in Section 18 of this specification.

A conformance analysis SHOULD assess each invariant independently using one of the following outcomes:

- `PASS` — the implementation preserves the invariant.
- `PARTIAL` — the invariant is preserved only under limited conditions or remains incompletely specified.
- `FAIL` — the implementation violates the invariant.

Conformance does not require adoption of the reference implementation, its software architecture, API design, or technology stack. An independently designed system may conform to DOLI if it preserves the protocol's legal and security invariants with equivalent assurance.

The resulting analysis SHOULD identify the public specification, legislation, API, or implementation version being evaluated so that the assessment remains reproducible as the external system evolves.
